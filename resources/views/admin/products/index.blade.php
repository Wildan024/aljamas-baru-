...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h2 class="text-xl font-bold text-[#163326]">Daftar Produk</h2>
        <p class="text-[#6B7280] text-sm mt-0.5">{{ $products->total() }} produk terdaftar</p>
    </div>
...
<div class="bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden">
    ...
    @if ($products->isEmpty())
        ...
        <div class="flex flex-col items-center justify-center py-20 text-center px-4">
            ...
            <p class="text-[#163326] font-semibold">Belum ada produk</p>
            <p class="text-[#6B7280] text-sm mt-1 mb-5">Mulai tambahkan produk pertama Anda.</p>
            <a href="{{ route('admin.products.create') }}"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-[#E5E7EB] bg-[#F7FAF8]">
                        <th class="text-left text-xs font-semibold text-[#6B7280] uppercase tracking-wider px-5 py-3.5">Produk</th>
                        ...
                        <tr class="hover:bg-[#F7FAF8] transition-colors duration-100">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-semibold text-[#163326] truncate max-w-[200px]">{{ $product->name }}</p>
                                        <p class="text-xs text-[#6B7280] truncate max-w-[200px]">/produk/{{ $product->slug }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 text-center">
                                <span class="text-[#1F2937] font-medium text-sm">{{ $product->sort_order }}</span>
                            </td>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST"
                                          ...
                                          onsubmit="return confirm('Hapus produk &quot;{{ addslashes($product->name) }}&quot;? Gambar produk juga akan dihapus permanen.')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
